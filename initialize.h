...
*everytime buffer management in,When the request is hit, the group抯 first team to move to the LRU, but look at this group if there has been 
...
struct request{
	long long int  time;                      //请求到达的时间，单位为us,这里和通常的习惯不一样，通常的是ms为单位，这里需要有个单位变换过程
